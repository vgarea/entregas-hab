  ...
  try {
    ...
    if (email !== currentUser[0].email) {
      ...
      await connection.query(
        `
        UPDATE users 
        SET name=?, email=?, lastUpdate=UTC_TIMESTAMP, lastAuthUpdate=UTC_TIMESTAMP, active=false, registrationCode=?, foto=?
        WHERE id=?
      `,
      ...
      await connection.query(
        `
      UPDATE users 
      SET name=?, email=?, foto=?, lastUpdate=UTC_TIMESTAMP
      WHERE id=?
    `,
