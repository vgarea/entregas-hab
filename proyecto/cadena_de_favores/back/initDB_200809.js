...
async function main() {
  try {
    ...
    await connection.query("DROP TABLE IF EXISTS favour_votes");
    await connection.query("DROP TABLE IF EXISTS user_votes");
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS favour_votes (
          id INTEGER PRIMARY KEY AUTO_INCREMENT,
          entry_id INTEGER NOT NULL,
          vote TINYINT NOT NULL,
          date DATETIME NOT NULL,
    ...
    for (let index = 0; index < favoursEntries; index++) {
      ...
      await connection.query(`
        INSERT INTO favours(location, description, category, deadline, creation_date, user_doer_id, user_maker_id )
        VALUES(
          "${faker.address.city()}",
    ...
    console.log("Metiendo datos de prueba en user_votes");
    const userVotesEntries = 100;

    for (let index = 0; index < userVotesEntries; index++) {
      const date = formatDateToDB(faker.date.recent());

      await connection.query(`
        INSERT INTO user_votes(entry_id, vote, date, user_id, lastUpdate)
        VALUES (
          "${random(1, userVotesEntries)}", 
          "${random(1, 5)}", 
          "${date}", 
          "${random(2, users + 1)}", 
          UTC_TIMESTAMP())
    ...
    console.log("Metiendo datos de prueba en favor_votes");
    const favourVotesEntries = 100;

    for (let index = 0; index < favourVotesEntries; index++) {
      const date = formatDateToDB(faker.date.recent());

      await connection.query(`
        INSERT INTO favour_votes(entry_id, vote, date, user_id, lastUpdate)
        VALUES (
          "${random(1, favourVotesEntries)}", 
          "${random(1, 5)}", 
          "${date}", 
          "${random(2, users + 1)}", 
          UTC_TIMESTAMP())
