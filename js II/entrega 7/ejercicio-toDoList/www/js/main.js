class TodoList {
    constructor(form, list, keyName) {
        // Nombre con el que guardar la lista en el localStorage
        this.keyName = keyName;

        // Lista de todos
        this.todos = [];

        // Miro a ver si hay todo guardados en el localStorage y si los hay los uso como todos iniciales
        const storedTodos = JSON.parse(window.localStorage.getItem(this.keyName));

        if(storedTodos) this.todos = storedTodos;
    ...
    attachEvents(){
        //Cuando se envía el formulario añade un nuevo todo con el texto que haya en el input
        ...
        //Cuando se pulse el botón de limpiar tareas se ejecute el método cleanTodoList
        ...
        this.deleteAllButton.addEventListener('click', (e) => {
            if(window.prompt('Escribe BORRAR para borrar todo los todos') === 'BORRAR') {
                this.deleteAllTodos();
            }
        });
    ...
    // Este método borra con los todos
    addTodo(text){
        if(text.lenght === 0) throw new Error('El texto del todo está vacío');
        //throw es como un return de un error
        const newTodo = {
            text: text,
            done: false,
        }
...
/*
    const myTodos = new TodoList('stored-todos');

    myTodos.addTodo('poner la lavadora');
    myTodos.addTodo('Ir al super');
    console.log(myTodos.todos);

    myTodos.toggleTodoStatus(0);
    console.log(myTodos.todos);

    myTodos.cleanTodoList();
    console.log(myTodos.todos);

    myTodos.deleteAllTodos();
*/